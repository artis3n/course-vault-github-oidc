...
Typically, `contents: read` to checkout the repository is the only permission you need unless you have additional use cases.
...
However, `sub`, or combining `sub` with additional claims, is the most common method to construct fine-grained access control.

### :keyboard: Activity: Fine-grained permissions - branches

Let's apply what we've learned to our next workflow.
We're going to follow the instructions from our previous activity, but this time you're going to bind the Vault role to the `main` branch.
...
1. After the previous activity, you should see a new workflow file in the repo: `.github/workflows/3-main-branch.yml`.
Open this file in your code editor.
1. Locate the step `name: Create an OIDC Role`.
1. Replace this step with the following code.
**Replace the `YOUR_REPO` section with the `org/repo` that applies to the repository you created from this course**.
...
1. After the previous activity, you should see a new workflow file in the repo: `.github/workflows/4-environment.yml`.
Open this file in your code editor.
1. There are two jobs in this workflow file! `staging` and `prod`.
...
This example will use a `stage4` branch.
    ```bash
...
However, for the sake of this course, we've configured Vault roles in the same workflow that consumed secrets.
...
- Use this [Terraform module](https://github.com/digitalocean/terraform-vault-github-oidc) to help manage your GitHub OIDC Vault role configurations.
